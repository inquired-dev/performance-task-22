...
const Settings = () => {
  ...
  const formik = useFormik({
    initialValues: {
      homework: 0,
      assessments: 0,
      quiz: 0, //initial values of quiz were not added after property was added
    },
    enableReinitialize: true,
    validationSchema: yup.object({
      homework: yup.number(),
      assessments: yup.number(),
      quiz: yup.number(), // same here, schema validation not added after property was added
    }),
  ...
  const getTotal = () =>
    Math.round(
      (formik.values.homework +
        ...
        formik.values.quiz) *
        100
    ); // this is where we needed to add together the total grade value but this code has a bug in it...
      ...
      >
        <Paper sx={{ padding: "24px" }}>
          ...
          <Box width="50%" margin="auto" padding="20px">
            ...
            >
              ...
              <TextField
                name="quiz"
                type="number"
                value={formik.values.quiz * 100} // this was a small bug that was causing issues with allowing the teacher to update grade values. This just needs to be changed to formik.values.quizzes so we are capturing the quiz values and not the assesments values twice
                InputProps={{ endAdornment: <Percent fontSize="small" /> }}
                onChange={handleChange}
                size="small"
                sx={{ width: "100px" }}
          ...
          <Box display="flex" justifyContent="space-between">
            <Button onClick={() => navigate("/calculator")} variant="outlined">
              Back
            </Button>
